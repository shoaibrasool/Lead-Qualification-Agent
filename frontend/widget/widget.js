(function () {
  ...
  var EMBEDDED_CSS = (
    ...
    '.lq-bubble{position:fixed;bottom:24px;right:24px;width:60px;height:60px;border-radius:50%;background:#0d9488;color:#fff;border:none;cursor:pointer;box-shadow:0 4px 12px rgba(13,148,136,0.35);display:flex;align-items:center;justify-content:center;z-index:999999;transition:transform .2s,box-shadow .2s;font-size:28px}' +
    '.lq-bubble:hover{transform:scale(1.08);box-shadow:0 6px 20px rgba(13,148,136,0.45)}' +
    ...
    '.lq-header{background:linear-gradient(135deg,#115e59,#0d9488);color:#fff;padding:14px 16px;flex-shrink:0;display:flex;justify-content:space-between;align-items:flex-start;gap:12px}' +
    ...
    '.lq-message{max-width:80%;padding:10px 14px;border-radius:16px;font-size:14px;line-height:1.5;word-wrap:break-word}' +
    '.lq-message.lq-user{align-self:flex-end;background:#0d9488;color:#fff;border-bottom-right-radius:4px}' +
    ...
    '.lq-message.lq-bot a{color:#0d9488;text-decoration:underline}' +
    '.lq-slot-buttons{align-self:flex-start;display:flex;flex-wrap:wrap;gap:8px;margin-top:4px}' +
    '.lq-slot-btn{padding:8px 16px;border-radius:20px;border:1px solid #0d9488;background:#fff;color:#0d9488;font-size:13px;font-weight:500;cursor:pointer;transition:background .15s,color .15s}' +
    '.lq-slot-btn:hover{background:#0d9488;color:#fff}' +
    ...
    '.lq-input:focus{border-color:#0d9488}' +
    '.lq-send{width:40px;height:40px;border-radius:50%;border:none;background:#0d9488;color:#fff;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .15s,transform .15s;flex-shrink:0}' +
    '.lq-send:hover{background:#0f766e;transform:scale(1.05)}' +
  ...
  function loadCSS() { return EMBEDDED_CSS; }
  ...
  LeadQualWidget.prototype._send = function (text) {
  ...
  LeadQualWidget.prototype._postMessage = function (text) {
    ...
    xhr.onload = function () {
      ...
      if (xhr.status === 200) {
        ...
        self._addMessage('Sorry, something went wrong.', 'bot');
        self._addRetryButton();
        self._scrollBottom();
      }
    };
  ...
  LeadQualWidget.prototype._addRetryButton = function () {
    var self = this;
    var container = document.createElement('div');
    container.className = 'lq-slot-buttons';
    var btn = document.createElement('button');
    ...
    btn.addEventListener('click', function () {
      container.remove();
      self._postMessage(self._lastSentText);
    });
    container.appendChild(btn);
    this.el.messages.appendChild(container);
  };
